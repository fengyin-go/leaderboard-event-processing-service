...
type Record012Handler interface{ Handle(string) string }
type Record012Processor struct{ prefix string }

func (p *Record012Processor) Handle(v string) string { return p.prefix + v }
func (s *EventStore) Record012(h Record012Handler, key string) string {
	if h == nil {
		return "missing"
	}
	return h.Handle(key)
