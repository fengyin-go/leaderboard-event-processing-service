...
type Record005Handler interface{ Handle(string) string }
type Record005Processor struct{ prefix string }

func (p *Record005Processor) Handle(v string) string { return p.prefix + v }
func (s *EventStore) Record005(h Record005Handler, key string) string {
	if h == nil {
		return "missing"
	}
	return h.Handle(key)
