...
func RunScenario025() (result string, err error) {
	s := eventstore.New()
	for i := 1; i <= 2; i++ {
		if err = s.Record025("member-25", i); err == nil {
			break
		}
	}
	return fmt.Sprintf("attempts=%d,writes=%d", s.Attempt("member-25"), len(s.Snapshot("member-25"))), err
}
