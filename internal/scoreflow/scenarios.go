...
func RunScenario011() (result string, err error) {
	s := eventstore.New()
	for i := 1; i <= 2; i++ {
		if err = s.Record011("member-11", i); err == nil {
			break
		}
	}
	return fmt.Sprintf("attempts=%d,writes=%d", s.Attempt("member-11"), len(s.Snapshot("member-11"))), err
}
