...
func RunScenario021() (result string, err error) {
	s := eventstore.New()
	first := s.Record021("member-21")
	second := s.Record021("member-21")
	return fmt.Sprintf("first=%t,second=%t,changes=%d", first, second, len(s.Snapshot("member-21"))), nil
}
