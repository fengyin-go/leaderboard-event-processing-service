...
func RunScenario004() (result string, err error) {
	s := eventstore.New()
	for i := 1; i <= 2; i++ {
		if err = s.Record004("member-4", i); err == nil {
			break
		}
	}
	return fmt.Sprintf("attempts=%d,writes=%d", s.Attempt("member-4"), len(s.Snapshot("member-4"))), err
}
