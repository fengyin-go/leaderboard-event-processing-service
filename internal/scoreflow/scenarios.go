...
func RunScenario014() (result string, err error) {
	s := eventstore.New()
	first := s.Record014("member-14")
	second := s.Record014("member-14")
	return fmt.Sprintf("first=%t,second=%t,changes=%d", first, second, len(s.Snapshot("member-14"))), nil
}
