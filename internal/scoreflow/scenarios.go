...
func RunScenario007() (result string, err error) {
	s := eventstore.New()
	first := s.Record007("member-7")
	second := s.Record007("member-7")
	return fmt.Sprintf("first=%t,second=%t,changes=%d", first, second, len(s.Snapshot("member-7"))), nil
}
