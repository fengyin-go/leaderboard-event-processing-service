...
func RunScenario018() (string, error) { return scenario(eventstore.New(), 18) }
...
func RunScenario028() (result string, err error) {
	s := eventstore.New()
	first := s.Record028("member-28")
	second := s.Record028("member-28")
	return fmt.Sprintf("first=%t,second=%t,changes=%d", first, second, len(s.Snapshot("member-28"))), nil
}
